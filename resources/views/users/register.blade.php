<x-layout>
    ...
    <div class="my-5 w-full min-h-min flex items-center justify-center">
        <div class="w-full">
            <div class="bg-white w-5/6 md:w-3/4 lg:w-2/3 xl:w-[500px] 2xl:w-[550px] mt-8 mx-auto px-16 py-8 rounded-lg shadow-md shadow-red-300">
                ...
                <p class="text-center text-sm text-gray-600 mt-2">Already have an account? <a href="/login" class="text-pink-400 hover:text-pinklue-700 hover:underline" title="Sign In">Sign in here</a></p>

                <form method="POST" action="/users" class="my-8 text-sm">
                    @csrf
                    <div class="flex flex-col my-4">
                        <label for="name" class="text-gray-700">Name</label>
                        <input type="text" name="name" id="name" class="mt-2 p-2 border border-gray-300 focus:outline-none focus:ring-0 focus:border-gray-300 rounded text-sm text-gray-900" placeholder="Enter your name" value="{{old('name')}}"/>
                        @error('name')
                    ...
                    <div class="flex flex-col my-4">
                        ...
                        <input type="email" name="email" id="email" class="mt-2 p-2 border border-gray-300 focus:outline-none focus:ring-0 focus:border-gray-300 rounded text-sm text-gray-900" placeholder="Enter your email"/ value="{{old('email')}}">
                        @error('email')
                        <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                      @enderror
                    </div>
